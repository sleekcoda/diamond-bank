jQuery(document).ready(function($) {
	...
	var errorLenght = $('#rc_rate_history_form').validate()
	$('#rc_rate_history_form').submit(function(e){
		e.preventDefault();
		if(errorLenght.errorList.length >= 1){
			$('#rc_rate_history_form').append('<div class="notice notice-error"><p>Error: Form validation error </p></div>');
			$('.notice').fadeOut(3000,null);
			return null;
		} else if(errorLenght.errorList.length == 0){
